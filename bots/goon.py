...
class BotPlayer:
            ...
            if bot_state and bot_state.get("holding"):
                if (
                    ...
                    if self.move_towards(controller, self.bot_id, tx, ty):
                        controller.trash(self.bot_id, tx, ty)
                    return False
            sx, sy = self.shop_pos[0]
        ...
        def cook_ing(ing_name: str):
            ...
            if ing_name in ["EGG", "MEAT"] and held_item.get("cooked_stage") != 1:
                if self.move_towards(
                    ...
                    if controller.place(
                        self.bot_id, self.cooker_pos[0][0], self.cooker_pos[0][1]
                    ):
                        self.cooking = (
                            held_item,
                            self.cooker_pos[0][0],
    ...
    def submit_plate(self, controller, held_item):
        """
        ...
        if held_item is None:
            px, py = self.current_order.plate_tracker.plate_pos
            if self.move_towards(controller, self.bot_id, px, py):
                controller.pickup(self.bot_id, px, py)
        else:
            # Move to submit location and submit
            sx, sy = self.submit_pos[0]
            if self.move_towards(controller, self.bot_id, sx, sy):
                if controller.submit(self.bot_id, sx, sy):
                    # Reset current order
                    self.current_order = ActiveOrder()
    ...
    def bot_turn(self, controller: RobotController, bot_id: int):
        ...
        if not self.current_order.order:
            ...
            self.submit_plate(controller, holding)
    ...
    def play_turn(self, controller: RobotController):
        ...
        self.bot_turn(controller, bots[1])
