...
class BotPlayer:
    ...
    def get_bfs_path_steps(
        ...
        queue = deque([start])
        visited = {start}
        came_from = {}  # (nx, ny) -> (prev_x, prev_y, dx, dy)
        ...
        while queue:
            ...
            if target_predicate(curr_x, curr_y, tile):
                ...
                curr = (curr_x, curr_y)
                while curr != start:
                    prev_x, prev_y, dx, dy = came_from[curr]
                    path.append((dx, dy))
                    curr = (prev_x, prev_y)
                return path[::-1]
                ...
                for dy in [-1, 0, 1]:
                    ...
                    if 0 <= nx < w and 0 <= ny < h and (nx, ny) not in visited:
                        ...
                        if (
                            ...
                            visited.add((nx, ny))
                            came_from[(nx, ny)] = (curr_x, curr_y, dx, dy)
                            queue.append((nx, ny))
    ...
    def init_claim_locations(self, bot_id: int, controller: RobotController):
        ...
        w, h = (
            controller.get_map(controller.get_team()).width,
            controller.get_map(controller.get_team()).height,
        )

        # flood fill from bot to find all reachable tiles
        reachable_tiles = set()
        queue = deque([(bot_x, bot_y)])
        visited = {(bot_x, bot_y)}

        # We need to respect bot collisions, but for initialization we might assume
        # bots move, so static map walkability is the main constraint.
        # Howevever, correct behavior is to track where we can go RIGHT NOW.

        # Get bot positions (same as get_bfs_path_steps)
        bot_positions = set()
        for b_id in controller.get_team_bot_ids(controller.get_team()):
            b = controller.get_bot_state(b_id)
            if b:
                bot_positions.add((b["x"], b["y"]))
        enemy = controller.get_enemy_team()
        for b_id in controller.get_team_bot_ids(enemy):
            b = controller.get_bot_state(b_id)
            if b:
                bot_positions.add((b["x"], b["y"]))
        ...
        while queue:
            ...
            for dx in [-1, 0, 1]:
                for dy in [-1, 0, 1]:
                    ...
                    if 0 <= nx < w and 0 <= ny < h and (nx, ny) not in visited:
                        if (
                            map_obj.is_tile_walkable(nx, ny)
                            and (nx, ny) not in bot_positions
                        ):
                            visited.add((nx, ny))
                            queue.append((nx, ny))

        def is_accessible(target_pos):
            # A resource is accessible if we can stand on any adjacent tile
            tx, ty = target_pos
            for dx in [-1, 0, 1]:
                for dy in [-1, 0, 1]:
                    scan_x, scan_y = tx + dx, ty + dy
                    if (scan_x, scan_y) in reachable_tiles:
                        return True
            return False
        ...
        for counter in self.all_counters:
            ...
            for claimed in all_claimed_counters:
                if counter in claimed:
                    ...
                    break
            if not is_claimed:
                if is_accessible(counter):
                    self.bot_claimed_locations[bot_id]["counters"].append(counter)

        # Select 1 non-claimed accessible stove
        all_claimed_stoves = [
    ...
    def find_empty_counter(self, controller: RobotController, bot_id: int):
        ...
        # Optimization: Use single BFS to find nearest reachable empty counter
        # 1. Identify all valid standing positions (neighbors of empty counters)
        goal_tiles = set()
        w, h = (
            controller.get_map(controller.get_team()).width,
            controller.get_map(controller.get_team()).height,
            ...
            for dx in [-1, 0, 1]:
                for dy in [-1, 0, 1]:
                    ...
                    if 0 <= nx < w and 0 <= ny < h:
                        goal_tiles.add((nx, ny))

        # 2. Run BFS with predicate checking if we are in goal_tiles
        def is_at_goal(x, y, tile):
            return (x, y) in goal_tiles

        path = self.get_bfs_path_steps(controller, (bot_x, bot_y), is_at_goal)

        if path is not None:
            # Calculate end position from path
            end_x, end_y = bot_x, bot_y
            for dx, dy in path:
                end_x += dx
                end_y += dy

            # Find the specific empty counter we are adjacent to
            best_counter = None
            min_dist = 9999

            for cx, cy in empty_counters:
                dist = max(abs(end_x - cx), abs(end_y - cy))
                if dist <= 1:
                    # Prefer the current one if it's the exact one we found (dist usually 1)
                    # Tie-breaking logic if needed, but any adjacent empty counter is fine
                    return (cx, cy)

            # Fallback if something went wrong map-wise (shouldn't happen)
            return self.find_closest(controller, bot_x, bot_y, empty_counters)

        # No path found to any empty counter
    ...
    def find_empty_cooker(self, controller: RobotController, bot_id: int):
        ...
        # Optimization: Use single BFS to find nearest reachable cooker with pan
        goal_tiles = set()
        w, h = (
            controller.get_map(controller.get_team()).width,
            controller.get_map(controller.get_team()).height,
        ...
        if path is not None:
            ...
            for cx, cy in cookers_with_pans:
                if max(abs(end_x - cx), abs(end_y - cy)) <= 1:
                    return (cx, cy)
    ...
    def find_cooker_without_a_pan(self, controller: RobotController, bot_id: int):
        ...
        # Optimization: Use single BFS to find nearest reachable cooker without pan
        goal_tiles = set()
        w, h = (
            controller.get_map(controller.get_team()).width,
            controller.get_map(controller.get_team()).height,
    ...
    def get_order(
        ...
        if self.should_get_order(controller):
            if active_orders is None:
                ...
                # Sort by urgency: shortest time remaining first
                active_orders = sorted(
                    active_orders, key=lambda o: o["expires_turn"], reverse=False
                )
                # logger(f"Bot {bot_id}: Found {len(active_orders)} active orders")

            # Score all viable orders
            scored_orders = []
            for order in active_orders:
                ...
                score = self.score_order(order, controller, bot_id)
                if score > 0:  # Valid and profitable
                    scored_orders.append((score, order))

            # Take highest scoring order
            if scored_orders:
    ...
    def is_good_order(self, order, controller: RobotController, bot_id: int):
        ...
        # Use Chebyshev distance (chess king movement): max(|dx|, |dy|)
        time_needed += max(
            abs(curloc["x"] - shop_loc[0]), abs(curloc["y"] - shop_loc[1])
        )
        time_needed += max(
            abs(counter1_loc[0] - shop_loc[0]), abs(counter1_loc[1] - shop_loc[1])
        )
        time_needed += max(
            abs(counter1_loc[0] - submit_loc[0]), abs(counter1_loc[1] - submit_loc[1])
        )
        ...
        for ingredient in order["required"]:
            if ingredient == "EGG":
                ...
                time_needed += max(
                    abs(shop_loc[0] - stove_loc[0]), abs(shop_loc[1] - stove_loc[1])
                )
                time_needed += max(
                    abs(stove_loc[0] - counter1_loc[0]),
                    abs(stove_loc[1] - counter1_loc[1]),
                )
                ...
                time_needed += max(
                    abs(shop_loc[0] - counter2_loc[0]),
                    abs(shop_loc[1] - counter2_loc[1]),
                )
                time_needed += max(
                    abs(counter2_loc[0] - counter1_loc[0]),
                    abs(counter2_loc[1] - counter1_loc[1]),
                )
                ...
                time_needed += max(
                    abs(counter2_loc[0] - stove_loc[0]),
                    abs(counter2_loc[1] - stove_loc[1]),
                )
                time_needed += max(
                    abs(stove_loc[0] - counter1_loc[0]),
                    abs(stove_loc[1] - counter1_loc[1]),
                ...
                # Shop -> Counter -> Shop
                time_needed += max(
                    abs(shop_loc[0] - counter1_loc[0]),
                    abs(shop_loc[1] - counter1_loc[1]),
                )
    ...
    def _calculate_ingredient_cost(self, ingredients: List[str]) -> int:
        """Calculate total ingredient cost"""
        costs = {"EGG": 20, "ONION": 30, "MEAT": 80, "NOODLES": 40, "SAUCE": 10}
        return sum(costs.get(ing, 0) for ing in ingredients)

    def _estimate_order_time(
        self, order, controller: RobotController, bot_id: int
    ) -> int:
        """Estimate turns needed to complete order (extracted from is_good_order logic)"""
        curloc = controller.get_bot_state(bot_id)
        if not curloc:
            return 999
    ...
    def score_order(self, order, controller: RobotController, bot_id: int) -> float:
        """
        Score an order based on multiple factors.
        Higher score = better order.
        Returns -1 if order is invalid/unprofitable.
        """
        # First check if order is even viable
        if not self.is_good_order(order, controller, bot_id):
            return -1.0

        # Calculate metrics
        ing_costs = self._calculate_ingredient_cost(order["required"])
        time_needed = self._estimate_order_time(order, controller, bot_id)
        turns_remaining = order["expires_turn"] - controller.get_turn()

        # Base score: reward per turn
        profit = order["reward"] + order["penalty"] - ing_costs
        value_per_turn = profit / max(time_needed, 1)

        # Urgency multiplier: exponential as deadline approaches
        # urgency = 1.0 when plenty of time, 2-5x when urgent
        time_buffer = turns_remaining - time_needed
        if time_buffer < 20:
            urgency = 2.0 + (20 - time_buffer) / 10  # 2x to 4x
        elif time_buffer < 50:
            urgency = 1.5
        else:
            urgency = 1.0
    ...
    def _cleanup_expired_order_ingredients(
        ...
        if self.bot_chopping.get(bot_id):
            ...
            # logger(
            #     f"Bot {bot_id}: Clearing choppin
            # g state at ({x}, {y}) due to expired order"
            # )
    ...
    def bot_turn(self, controller: RobotController, bot_id: int):
        ...
        if switch_info and switch_info.get("my_team_switched"):
            ...
            and switch_info.get("turn") >= (switch_info.get("window_end_turn") - 30)
        ):
