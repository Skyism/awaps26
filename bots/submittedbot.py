...
class BotPlayer:
    def __init__(self, map_copy):
        ...
        if LOGGING_ENABLED:
            safe_open("tmp/goon.txt", "w").close()
    ...
    def find_empty_counter(self, controller: RobotController, bot_id: int):
        ...
        if not res:
            ...
            logger(f"FALLBACK COUNTER {self.all_counters[0]}")
            return self.all_counters[0] if self.all_counters else None
        cx, cy = self.find_closest(controller, bot_x, bot_y, res)
        logger(f"closest counter {cx, cy}")
        return (cx, cy)
